...
<div class="app">
  ...
  <div class="plots">
    <div class="left-col">
      ...
      <div class="panel">
        <div class="panel-title">
          <span>V&ndash;I phasor at z = z<sup>*</sup></span>
          <span class="legend">
            <span class="legend-chip" style="color:var(--muted)"><span class="swatch"></span>V/I = &plusmn;Z<sub>0</sub></span>
          </span>
        </div>
        <div class="canvas-wrap"><canvas id="cv-phasor"></canvas></div>
      </div>
    </div>
    <div class="panel">
      <div class="panel-title">
        <span>3D View — line as cylinder, V (top) and I (front) waves &nbsp;<span style="color:var(--muted);font-weight:400">(drag to rotate, double-click to toggle auto-rotation)</span></span>
        <span class="legend">
          <span class="legend-chip" style="color:#ffb454"><span class="swatch"></span>V<sup>+</sup></span>
          <span class="legend-chip" style="color:#c2670f"><span class="swatch"></span>V<sup>-</sup></span>
          <span class="legend-chip" style="color:#ffe2a8"><span class="swatch"></span>V<sub>tot</sub></span>
          <span class="legend-chip" style="color:#41d6d3"><span class="swatch"></span>I<sup>+</sup></span>
          <span class="legend-chip" style="color:#1c8786"><span class="swatch"></span>I<sup>-</sup></span>
          <span class="legend-chip" style="color:#bff0ef"><span class="swatch"></span>I<sub>tot</sub></span>
        </span>
      </div>
  ...
  <div class="controls">
    ...
    <div class="ctl-group">
      ...
      <div class="row"><label>z<sup>*</sup>/&ell;</label>
        <input id="rng-zs-slice" type="range" min="0" max="1" step="0.01" value="0.5">
        <span class="val" id="val-zs-slice">0.50</span>
      </div>
    </div>
...
<script>
  ...
  const COL = {
    ...
    // V family (orange shades) and I family (cyan shades)
    Vfwd: "#ffb454",
    Vrev: "#c2670f",
    Vtot: "#ffe2a8",
    Ifwd: "#41d6d3",
  ...
  function precompute(params) {
    const isSine = params.signal === "sine";
    const Nt = isSine ? 4096 : 2048;
    const Nz = 200;
    const tau = params.tau;
    const Tw = isSine ? 14 * tau : 6 * tau;
    const dt = Tw / Nt;
  ...
  function readParams() {
    ...
    const zSlice = +document.getElementById("rng-zs-slice").value;
    return {
      signal: sig, Z0: Z0_FIXED, length: len, vp, zs, rL, xL, xC, loadKind,
      tRise: tRiseFrac * tau,
      sigma: sigFrac * tau,
      freq, fref, tau, zSlice,
    };
  }
  ...
  // One-point perspective. Negative az puts +x_world (z=L end) closer to
  // the camera. View is mutable: drag the 3D canvas to rotate; otherwise it
  // auto-rotates slowly to highlight depth.
  const VIEW = { az: -50 * PI / 180, el: 18 * PI / 180, camDist: 5.0,
  ...
  function project(x, y, w) {
    ...
    const y2 = ce * y - se * w1;
    const w2 = se * y + ce * w1;
    const d = Math.max(0.4, VIEW.camDist + w2);
    const persp = VIEW.camDist / d;
    return [x1 * persp, y2 * persp];
  }
  ...
  function draw3D() {
    ...
    ctx.lineWidth = 1;
    // Rings at evenly spaced positions to make the foreshortening readable
    const ringX = [0, Lworld * 0.25, Lworld * 0.5, Lworld * 0.75, Lworld];
    ...
    // V curves: top of cylinder (w=0, y = Rcyl + V*sV) — orange family
    trace3((iz) => [Rcyl + grid.Vplus[iz][it] * sV, 0], COL.Vfwd, 0.95, 1.8);
    trace3((iz) => [Rcyl + grid.Vminus[iz][it] * sV, 0], COL.Vrev, 0.95, 1.8);
    trace3((iz) => [Rcyl + (grid.Vplus[iz][it] + grid.Vminus[iz][it]) * sV, 0], COL.Vtot, 1.0, 2.4);

    // I curves: front of cylinder (closer to camera: w < 0) — cyan family
    trace3((iz) => [0, -(Rcyl + (grid.Vplus[iz][it] / Z0_FIXED) * sI)], COL.Ifwd, 0.95, 1.8);
    trace3((iz) => [0, -(Rcyl + (-grid.Vminus[iz][it] / Z0_FIXED) * sI)], COL.Irev, 0.95, 1.8);
    trace3((iz) => [0, -(Rcyl + ((grid.Vplus[iz][it] - grid.Vminus[iz][it]) / Z0_FIXED) * sI)], COL.Itot, 1.0, 2.4);

    // Slice plane: ring at z = z* showing the phasor pickoff location
    {
      const zStar = state.params.length * (state.params.zSlice ?? 0.5);
      const xStar = (zStar / state.params.length) * Lworld;
      ctx.strokeStyle = COL.tot;
      ...
      for (let k = 0; k <= N; k++) {
        const th = (k / N) * TWO_PI;
        const yy = Rcyl * Math.cos(th), ww = Rcyl * Math.sin(th);
        const [px, py] = S(xStar, yy, ww);
        if (k === 0) ctx.moveTo(px, py); else ctx.lineTo(px, py);
      }
      ctx.stroke();
      ctx.globalAlpha = 1;
      const [pa, pb] = S(xStar, Rcyl + 0.18, 0);
      ctx.fillStyle = COL.tot;
      ctx.fillText("z*", pa, pb);
    }
  ...
  function drawPhasor() {
    ...
    const x0 = M.l, x1 = w - M.r, y0 = M.t, y1 = h - M.b;
    drawAxes(ctx, x0, y0, x1, y1, { nx: 6, ny: 6 });
    const grid = state.grid;
    const Vmax = state.yMax;
    const Imax = state.iMax;
    const cx = (x0 + x1) / 2, cy = (y0 + y1) / 2;
    const halfW = (x1 - x0) / 2, halfH = (y1 - y0) / 2;
    const Vmap = (V) => cx + (V / Vmax) * halfW;
    const Imap = (I) => cy - (I / Imax) * halfH;

    // Reference lines: I = +V/Z0 (forward matched) and I = -V/Z0 (reverse)
    ctx.strokeStyle = COL.muted; ctx.setLineDash([2, 4]); ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(Vmap(-Vmax), Imap(-Vmax / Z0_FIXED));
    ctx.lineTo(Vmap(+Vmax), Imap(+Vmax / Z0_FIXED));
    ctx.moveTo(Vmap(-Vmax), Imap(+Vmax / Z0_FIXED));
    ctx.lineTo(Vmap(+Vmax), Imap(-Vmax / Z0_FIXED));
    ctx.stroke();
    ctx.setLineDash([]);
    ...
    ctx.fillText("V", x1 - 12, cy - 4);
    ctx.fillText("I", cx + 4, y0 + 10);
    ctx.fillText("+" + Vmax.toPrecision(2), x1 - 30, y1 + 14);
    ctx.fillText("-" + Vmax.toPrecision(2), x0, y1 + 14);

    // Slice index
    const zFrac = state.params.zSlice ?? 0.5;
    const izStar = Math.max(0, Math.min(grid.Nz - 1, Math.round(zFrac * (grid.Nz - 1))));

    // Trail: trace from t_now backward by ~1.2*tau
    const trailDt = 1.2 * state.params.tau;
    const itNow = Math.max(0, Math.min(grid.Nt - 1, Math.round(state.tNow / grid.dt)));
    const itStart = Math.max(0, Math.round((state.tNow - trailDt) / grid.dt));
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    let started = false;
    for (let it = itStart; it <= itNow; it++) {
      const V = grid.Vplus[izStar][it] + grid.Vminus[izStar][it];
      const I = (grid.Vplus[izStar][it] - grid.Vminus[izStar][it]) / Z0_FIXED;
      const px = Vmap(V), py = Imap(I);
      if (!started) { ctx.moveTo(px, py); started = true; }
      else ctx.lineTo(px, py);
    }
    ctx.strokeStyle = COL.Vtot;
    ctx.globalAlpha = 0.55;
    ctx.stroke();
    ctx.globalAlpha = 1;

    // Forward and reverse phasor arrows from origin (V+, I+) and (V-, I-)
    const Vp = grid.Vplus[izStar][itNow];
    const Vm = grid.Vminus[izStar][itNow];
    const Vt = Vp + Vm;
    const It = (Vp - Vm) / Z0_FIXED;

    function arrow(Vx, Iy, color) {
      ctx.strokeStyle = color; ctx.fillStyle = color; ctx.lineWidth = 1.8;
      ctx.beginPath();
      ctx.moveTo(Vmap(0), Imap(0));
      ctx.lineTo(Vmap(Vx), Imap(Iy));
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(Vmap(Vx), Imap(Iy), 3.0, 0, TWO_PI);
      ctx.fill();
    }
    arrow(Vp, Vp / Z0_FIXED, COL.Vfwd);
    arrow(Vm, -Vm / Z0_FIXED, COL.Vrev);
    arrow(Vt, It, COL.tot);

    // Slice readout
    ctx.fillStyle = COL.muted;
    ctx.fillText("z* = " + (zFrac).toFixed(2) + "·ℓ", x0 + 4, y0 + 12);
  }
  ...
  // ---------- 3D camera interaction ----------
  {
    ...
    window.addEventListener("mousemove", (e) => {
      ...
      lastX = e.clientX; lastY = e.clientY;
      VIEW.az += dx * 0.006;
      VIEW.el = Math.max(-1.4, Math.min(1.4, VIEW.el + dy * 0.006));
    });
  ...
  function frame(now) {
    ...
    if (VIEW.autoRotate && !VIEW.dragging) {
      // Slow oscillating rotation around the nominal head-on view.
      VIEW.az += dt * (8 * PI / 180);
      // Gentle vertical bob to expose top/bottom of cylinder
      VIEW.el = 18 * PI / 180 + 0.18 * Math.sin(performance.now() * 0.0005);
    }
    if (state.running) {
      state.tNow += dt * state.speed * state.params.tau;
      if (state.params.signal === "sine") {
        // Skip the turn-on transient and loop over an integer number of
        // sine periods so the playback wraps without a visible jump.
        const Tsin = 1 / state.params.freq;
        const tStart = 0.5 * state.grid.Tw;
        const remaining = state.grid.Tw - tStart;
        const N = Math.max(1, Math.floor(remaining / Tsin));
        const tEnd = tStart + N * Tsin;
        if (state.tNow >= tEnd || state.tNow < tStart) state.tNow = tStart;
      } else if (state.tNow > state.grid.Tw) {
        state.tNow = 0;
      }
      const slider = document.getElementById("rng-time");
  ...
  // z-slice doesn't need recompute (just re-reads grid), but we use bindRange for uniformity.
  document.getElementById("rng-zs-slice").addEventListener("input", (e) => {
    document.getElementById("val-zs-slice").textContent = (+e.target.value).toFixed(2);
    if (state.params) state.params.zSlice = +e.target.value;
  });
